...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        int ReloadType_TypeID = -1;
		int ReloadType_EnumRef = -1;
        
        public void PushReloadType(RealStatePtr L, ReloadType val)
        {
            if (ReloadType_TypeID == -1)
            {
			    bool is_first;
                ReloadType_TypeID = getTypeId(L, typeof(ReloadType), out is_first);
				
				if (ReloadType_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(ReloadType));
				    ReloadType_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, ReloadType_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, ReloadType_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for ReloadType ,value="+val);
            }
			
			LuaAPI.lua_getref(L, ReloadType_EnumRef);
        ...
        public void Get(RealStatePtr L, int index, out ReloadType val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != ReloadType_TypeID)
				{
				    throw new Exception("invalid userdata for ReloadType");
				}
                ...
                {
                    throw new Exception("unpack fail for ReloadType");
                }
				val = (ReloadType)e;
            ...
            {
                val = (ReloadType)objectCasters.GetCaster(typeof(ReloadType))(L, index, null);
            }
        }
		
        public void UpdateReloadType(RealStatePtr L, int index, ReloadType val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for ReloadType ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(ReloadType[]))
			{
			    ReloadType[] array = obj as ReloadType[];
				translator.PushReloadType(L, array[index]);
				return true;
			}
